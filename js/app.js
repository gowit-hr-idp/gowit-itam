...
async function navigateTo(page) {
  ...
  switch (page) {
    ...
    case 'azure-costs':     refreshAllCategoryDropdowns(); await loadAzureCosts(); if (typeof loadAzureResourceUsage === 'function') loadAzureResourceUsage(); if (typeof collapseAzCostList === 'function') collapseAzCostList(); break;
...
async function showAssetDetail(id) {
  ...
  document.getElementById('assetDetailContent').innerHTML = `
    <div class="detail-section" style="border-color:#ddd6fe;background:#faf5ff">
      <h4 style="color:#7c3aed"><i class="fas fa-qrcode mr-1"></i>자산 식별 정보 (자산번호 · 제조사 · 모델명 · 구입일자)</h4>
      <div class="detail-value font-mono font-semibold text-violet-700" style="word-break:break-all;font-size:0.95rem">${[asset.asset_no || '-', asset.manufacturer || '-', asset.model_name || '-', asset.purchase_date || '-'].join(' / ')}</div>
    </div>
    <div class="detail-section">
